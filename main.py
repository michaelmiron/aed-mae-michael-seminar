...
def main(args):
    ...
    dataset_test = Subset(dataset_test, list(range(1000)))
    ...
    if args.run_type == "train":
        ...
        student = torch.load(args.output_dir + "/checkpoint-best-student.pth", map_location=torch.device('cpu'), weights_only=False)['model']
        teacher = torch.load(args.output_dir + "/checkpoint-best.pth", map_location=torch.device('cpu'), weights_only=False)['model']
        for key in student:
            if 'student' in key:
